...
def run_tournament_job(
    ...
    logger.info(f"🚀 Thread開始: job_id={job_id}, game_id={game_id}")
...
async def start_tournament_job(
    ...
    try:
        ...
        # バックグラウンドジョブを開始
        # Railway環境ではBackgroundTasksが動作しないため、直接Threadで実行
        import threading
        
        logger.info(f"🏀 大会ジョブ開始（Thread強制実行）: {job_id} - 大会ID: {req.game_id}")
        
        # Threadで直接実行（Railway環境対応）
        ...
        # ジョブファイルを更新（Thread開始確認）
        meta["message"] = "Threadでジョブを開始しました..."
        meta["updated_at"] = datetime.now().isoformat()
        with open(job_file, "w", encoding="utf-8") as f:
            json.dump(meta, f, ensure_ascii=False, indent=2)
