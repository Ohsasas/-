...
async def start_tournament_job(
    ...
    try:
        ...
        # バックグラウンドジョブを開始
        # BackgroundTasksが動作しない環境用に、Threadで実行するオプションも用意
        import threading
        
        # まずBackgroundTasksで試行
        try:
            ...
            logger.info(f"🏀 大会ジョブ開始（BackgroundTasks）: {job_id} - 大会ID: {req.game_id}")
        except Exception as e:
            # BackgroundTasksが失敗した場合、Threadで実行
            logger.warning(f"⚠️ BackgroundTasks失敗、Threadで実行: {e}")
            thread = threading.Thread(
                target=run_tournament_job,
                args=(job_id, req.game_id, req.jba_credentials, req.generate_pdf),
                daemon=True
            )
            thread.start()
            logger.info(f"🏀 大会ジョブ開始（Thread）: {job_id} - 大会ID: {req.game_id}")
